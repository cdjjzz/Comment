...
Ext.define('MessageContainer', {  
    ...
    tpl : [  
            ...
            '<div class="l-im-message-header l-im-message-header-{source}">{from}--{to} {timestamp}</div>',  
    ...
    initComponent : function() {  
        ...
        me.messageModel = Ext.define('Leetop.im.MessageModel', {  
                    ...
                    fields : ['from','to', 'timestamp', 'content', 'source']  
                });  
...
Ext.onReady(function() {  
    ...
    function initWebSocket() {  
        if (window.WebSocket) {  
        	//初始化
            websocket = new WebSocket(encodeURI('ws://localhost:18001/Comment/webmessage'));  
            //打开一个连接
            ...
            websocket.onmessage = function(message) {  
                ...
                } else if (message.type == 'onlin') {  
                    //获取在线用户列表  
                    var root = onlineUser.getRootNode();  
                    Ext.each(message.users,function(user){  
